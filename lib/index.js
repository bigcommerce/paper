...
internals.loadHelpers = function (context, translations) {
    ...
    Handlebars.registerHelper('cdn', function(assetPath) {
        ...
        if (assetPath.substr(-4) === '.css') {
            ret = Path.join(context.cdn_url_with_settings_hash, assetPath);
        } else {
            ret = Path.join(context.cdn_url, assetPath);
        }
    ...
    Handlebars.registerHelper('@image', function (image, preset, defaultImage) {
