...
internals.loadHelpers = function (context, translations) {
     ...
     * {{#anyInclude fonts, "Roboto" }} ... {{/anyInclude}}
     * {{#anyInclude font_path, "Roboto" }} ... {{/anyInclude}}
     */
    Handlebars.registerHelper('anyInclude', function(value, targetValue) {
        var targetValues = Array.prototype.slice.call(arguments, 1, -1),
            options = _.last(arguments),
            anyInclude,
            isIncluded;

        // Define predicate method
        anyInclude = function anyInclude(value, targetValues) {
            return _.any(targetValues, function(targetValue) {
                return _.includes(value, targetValue);
            });
        };

        if (_.isObject(value)) {
            // If passed in parameter is a hash or array, shallowly compare each value
            isIncluded = _.any(value, function(value) {
                return anyInclude(value, targetValues);
            });
        } else {
            // Otherwise, perform a direct comparison
            isIncluded = anyInclude(value, targetValues);
        }
